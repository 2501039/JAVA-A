import java.util.Scanner;

public class Input {
    private static Scanner keyboard = new Scanner(System.in);

    public static float[] readData(String[][] customers) {
        float[] data = new float[customers.length];
        for (int i = 0; i < customers.length; i++) {
            data[i] = readSingleData(customers[i][1]);
        }
        return data;
    }

    private static float readSingleData(String name) {
        float temp;
        while (true) {
            System.out.printf("%s 고객의 수도 사용량 입력: ", name);
            if (keyboard.hasNextFloat()) {
                temp = keyboard.nextFloat();
                if (temp >= 0.0f && temp <= 999.9f) {
                    return temp;
                } else {
                    System.err.println("ERROR: 0.0 ~ 999.9 사이 허용!");
                }
            } else {
                System.err.println("ERROR: 숫자를 입력");
                keyboard.next();
            }
        }
    }

    public static void preProcess(String[] customer) {
        if (customer[2].equals("1")) {
            customer[3] = "가정용";
        } else if (customer[2].equals("2")) {
            customer[3] = "영업용";
        } else if (customer[2].equals("3")) {
            customer[3] = "공장용";
        } else if (customer[2].equals("4")) {
            customer[3] = "관공서용";
        } else if (customer[2].equals("5")) {
            customer[3] = "군기관용";
        } else {
            customer[3] = "";
        }
    }
}
